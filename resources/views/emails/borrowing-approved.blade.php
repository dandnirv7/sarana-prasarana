...
<div class="container">
    ...
    <div class="content">
        <p>Halo {{ $user->name }},</p>
        <p>Peminjaman aset <strong>{{ $asset->name }}</strong> telah disetujui.</p>
        <p>Tanggal Pinjam: {{ $borrowing->borrow_date }}<br>
           Tanggal Kembali: {{ $borrowing->return_date ?? '-' }}</p>
        <p>Silakan gunakan aset sesuai jadwal.</p>
    </div>
    <div class="footer">
